...
# find a winning board, either by finding a row, column or diagonal with all X's
def find_winning_board(final_list, numbers):
    for number in numbers:
        for i in range(len(final_list)):
            for y in range(len(final_list[i])):
                row_counter = 0
                if all_equal(final_list[i][y]):
                    return final_list[i]
                for z in range(len(final_list[i][y])):
                    ...
                    if final_list[i][z][y] == "X":
                        row_counter+=1
                    if row_counter == 5:
                        return calc_remaining_board_value(final_list[i]) * numbers[(numbers.index(number)-1)]

print(find_winning_board(final_list, numbers))
